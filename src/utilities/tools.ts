...
export const imageFileFilter = (req, file, callback) => {
  if (
    !file.originalname.normalize().match(/\.(jpeg|png|gif|jpg|JPG|PNG|JPEG)$/)
  ) {
    return callback(
      new BadRequestException('Only image files are allowed! jpeg|png|gif|jpg'),
      false,
    );
  }
  callback(null, true);
...
export async function hashImage(file: Express.Multer.File) {
  ...
  const hash: string = await new Promise((resolve, reject) =>
    imageHash('./tmp/' + filename, 16, false, (error, hash1) => {
      if (error) {
        reject(error);
      } else {
        resolve(hash1);
  ...
  try {
    ...
    unlink('./tmp/' + filename, (err) => {
      if (err) {
        throw new NotFoundException(
          `Couldn't delete file: ${filename}, Error is : ${err}`,
        );
      }
    });
  } catch (err) {
    ...
    if (err.code == 'EEXIST') {
      ...
      const colors1 = (await getImageColors(filename, mime.lookup(extension)))
        .map((color) => {
          return color.hex();
        })
        .toString();
      const colors2 = (
        await getImageColors(
          '../files/' + hashedname,
          mime.lookup(extname(hashedname)),
        )
      )
        .map((color) => {
          return color.hex();
      ...
      if (colors1 != colors2) {
        ...
        try {
          ...
          if (err?.code != 'EEXIST') {
            throw new NotFoundException(
              `Couldn't copy file: ${filename}, Error is : ${err}`,
            );
          }
        }
      }
      ...
      throw new NotFoundException(
        `Couldn't find file: ${filename}, Error is : ${err}`,
      );
    }
  }
...
export async function fetchWithoutError(
  url: string,
  options: any,
