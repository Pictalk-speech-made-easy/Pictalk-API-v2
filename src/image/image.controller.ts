...
export class ImageController {
  ...
  async searchImages(
    ...
    const flickrRes = lastValueFrom(
      this.httpService.get(this.generateLinks(filterDto, 'flickr')),
    )
      .then((flickrRes) => {
      ...
      .catch((error) => {
        if (error.response.status == 429) {
          this.logger.verbose('[ERR] FLICKR TOO MUCH REQUESTS');
        }
      });
    ...
    const unsplashRes = lastValueFrom(
      ...
      .catch((error) => {
        if (error.response.status == 429) {
          this.logger.verbose('[ERR] UNSPLASH TOO MUCH REQUESTS');
        }
      });

    const pexelsRes = client.photos
      .search({
        query: filterDto.search,
        size: 'small',
        locale: languageMapping[`${filterDto.language}`],
        per_page: filterDto.perPage ? filterDto.perPage : this.per_page_default,
      })
      .then((pexelsRes: any) => {
      ...
      .catch((error) => {
        if (error.response.status == 429) {
          this.logger.verbose('[ERR] PEXELS TOO MUCH REQUESTS');
        }
      });

    const pixabayRes = lastValueFrom(
      this.httpService.get(this.generateLinks(filterDto, 'pixabay')),
    )
      .then((pixabayRes) => {
      ...
      .catch((error) => {
        if (error.response.status == 429) {
          this.logger.verbose('[ERR] PIXABAY TOO MUCH REQUESTS');
        }
      });
