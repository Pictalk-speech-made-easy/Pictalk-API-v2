...
export class CollectionService {
  ...
  async createCollection(
    ...
    createCollectionDto.collectionIds = await this.verifyOwnership(
      createCollectionDto.collectionIds,
      user,
    );
  ...
  async modifyCollection(
    ...
    if (collection.userId === user.id || index != -1) {
      modifyCollectionDto = await this.verifyOwnership(
        modifyCollectionDto,
        user,
      );
      ...
      return this.collectionRepository.modifyCollection(
        collection,
        modifyCollectionDto,
        user,
        filename,
      ...
      throw new UnauthorizedException(
        `User '${user.username}' is not authorized to modify this collection`,
      );
    }
  }

  async shareCollectionVerification(
    id: number,
    user: User,
    ...
    let sharers = await Promise.all(
      multipleShareCollectionDto.usernames.map(async (username) => {
    ...
    multipleShareCollectionDto.usernames = sharers.map((sharer) => {
    ...
    if (sharers.length > 0) {
      ...
      if (collection) {
        ...
        if (
          multipleShareCollectionDto.role === 'editor' &&
          !(collection.userId === user.id || editor != -1)
        ) {
          throw new UnauthorizedException(
            `${user.username} cannot share to ${sharers} as editor being a viewer youself`,
          );
        }
        ...
        for (let sharer of sharers) {
          const sharedWithMe = await this.getCollectionById(
            sharer.shared,
            sharer,
          );
          ...
          if (directSharer != -1) {
            const sharerRoot = await this.getCollectionById(
              sharer.root,
              sharer,
            );
          ...
          if (multipleShareCollectionDto.access == 1) {
            if (
              collection.editors.indexOf(sharer.username) == -1 &&
              collection.viewers.indexOf(sharer.username) == -1
            ) {
              const notification = await this.createNotif(
                collection,
                user,
  ...
  async shareCollectionById(
    ...
    try {
      collection.pictos.map((picto) =>
        this.collectionRepository.sharePictoFromDto(
          picto,
          multipleShareCollectionDto,
        ),
      );
    ...
    try {
      collection = await this.collectionRepository.shareCollectionFromDto(
        collection,
        multipleShareCollectionDto,
      );
    } catch (error) {}
  ...
  async publishCollectionById(
    collectionId: number,
    publicCollectionDto: publicCollectionDto,
    user: User,
  ): Promise<Collection> {
    let collection = await this.getCollectionById(collectionId, user);
    if (collection.userId === user.id) {
      try {
        collection.collections.map((collection) =>
          this.publishCollectionById(collection.id, publicCollectionDto, user),
        );
      } catch (error) {}
      try {
        collection.pictos.map((picto) =>
          this.collectionRepository.publishPicto(
            picto,
            publicCollectionDto.publish,
            user,
          ),
        );
      } catch (error) {}
      try {
        collection = await this.collectionRepository.publishCollection(
          collection,
          publicCollectionDto.publish,
          user,
        );
  ...
  async verifyOwnership(verificationDto: any, user: User) {
    try {
      for (var i = 0; i < verificationDto.collectionIds.length; i++) {
        try {
          const collection = await this.getCollectionById(
            verificationDto.collectionIds[i],
            user,
          );
        } catch (error) {
          i = i - 1;
    ...
    try {
      for (var i = 0; i < verificationDto.pictoIds.length; i++) {
        try {
          const picto = await this.pictoService.getPictoById(
            verificationDto.pictoIds[i],
            user,
          );
        } catch (error) {
          i = i - 1;
  ...
  async copyPicto(fatherId: number, picto: Picto, user: User): Promise<number> {
    ...
    if (
      ...
      const copiedPicto = await this.pictoService.createPicto(
        createPictoDto,
        user,
        picto.image,
      );
        ...
        const createCollectionDto: createCollectionDto = {
          ...
          collectionIds: await Promise.all(
            collection.collections.map((child) => {
              return this.copyCollection(collection.id, child.id, user);
            }),
          ),
          pictoIds: await Promise.all(
            collection.pictos.map((child) => {
              return this.copyPicto(collection.id, child, user);
            }),
          ),
          fatherCollectionId: fatherId,
          share: 0,
        };
        const copiedCollection = await this.createCollection(
          createCollectionDto,
          user,
          collection.image,
        );
  ...
  async moveToCollection(
    user: User,
    moveToCollectionDto: MoveToCollectionDto,
    fatherCollectionId: number,
  ): Promise<Collection> {
    const targetCollection = await this.getCollectionById(
      moveToCollectionDto.targetCollectionId,
      user,
    );
    if (moveToCollectionDto.sourceCollectionId) {
      const modifyCollection = new modifyCollectionDto();
      modifyCollection.collectionIds = targetCollection.collections.map(
        (collection) => collection.id,
      );
      modifyCollection.collectionIds.push(
      ...
      await this.collectionRepository.modifyCollection(
        targetCollection,
        modifyCollection,
        user,
        null,
      ...
      const modifyCollection = new modifyCollectionDto();
      modifyCollection.pictoIds = targetCollection.pictos.map(
        (collection) => collection.id,
      );
