...
export class CollectionController {
  ...
  async getLevelCollections(): Promise<levelCollectionDto> {
    ...
    if (parseInt(process.env.LEVEL_A_COLLECTION_ID)) {
      collectionArray.levelA = await this.collectionService.getCollectionById(
        parseInt(process.env.LEVEL_A_COLLECTION_ID),
        new User(),
      );
    }
    if (parseInt(process.env.LEVEL_B_COLLECTION_ID)) {
      collectionArray.levelB = await this.collectionService.getCollectionById(
        parseInt(process.env.LEVEL_B_COLLECTION_ID),
        new User(),
      );
    }
    if (parseInt(process.env.LEVEL_C_COLLECTION_ID)) {
      collectionArray.levelC = await this.collectionService.getCollectionById(
        parseInt(process.env.LEVEL_C_COLLECTION_ID),
        new User(),
      );
    }
  ...
  getPublicCollections(
    ...
    if (SearchCollectionDto.per_page) {
      if (
        SearchCollectionDto.per_page >= 5 &&
        SearchCollectionDto.per_page <= 100
      ) {
        SearchCollectionDto.page = SearchCollectionDto.page | 0;
      } else {
        throw new BadRequestException(
          `per_page must be an integer greater or equal to 5 and inferior or equal to 100`,
        );
      }
    }
  ...
  async copyCollection(
    ...
    const copiedId = await this.collectionService.copyCollection(
      copyCollectionDto.fatherCollectionId,
  ...
  @ApiOperation({
    summary:
      'share a collection with another user, with readonly or editor role',
  })
  async shareCollectionById(
    ...
    if (multipleShareCollectionDto.access) {
      this.logger.verbose(
        `User "${user.username}" sharing Collection with id ${id} to Users ${multipleShareCollectionDto.usernames} as ${multipleShareCollectionDto.role}`,
      );
    } else {
      this.logger.verbose(
  ...
  publishCollectionById(
    ...
    if (user.admin === true) {
      ...
      throw new UnauthorizedException(
        `User ${user.username} is not admin, only admins can make a collection public`,
      );
    }
  }
  @UseGuards(UserGuard)
  ...
  async createCollection(
    ...
    if (!file) {
      this.logger.verbose(
        `User "${user.username}" Made a bad request that doesn't contain a file`,
      );
      ...
      if (IsValid(createCollectionDto.meaning, createCollectionDto.speech)) {
        if (createCollectionDto.fatherCollectionId != user.shared) {
          ...
          this.collectionService.modifyCollection(
            createCollectionDto.fatherCollectionId,
            user,
            modifyCollectionDto,
            null,
          ...
          if (createCollectionDto.share != 0) {
            ...
            this.logger.verbose(
              `Auto sharing collection "${collection.id}" with viewers and editors`,
            );
          }
          return collection;
        } else {
          this.logger.verbose(
            `User "${user.username}" tried to create collection into shared collections`,
          );
          throw new ForbiddenException(
            `You cannot create a collection into your shared collection`,
          );
        }
      } else {
        this.logger.verbose(
          `User "${user.username}"Made a bad request where Object has either invalid attributes or "meaning" and "speech" don't have the same length`,
        );
        throw new BadRequestException(
          `Object is invalid, should be "{language <xx-XX> : text <string>} and both should have same length`,
        );
      }
    }
  }
  ...
  deleteCollection(
    ...
    this.logger.verbose(
      `User "${user.username}" deleting Collection with id ${deleteCollectionDto.collectionId}`,
    );
  ...
  async modifyCollection(
    ...
    if (IsValid(modifyCollectionDto.meaning, modifyCollectionDto.speech)) {
      this.logger.verbose(
        `User "${user.username}" Modifying Collection with id ${id}`,
      );
      if (file) {
        const filename = await hashImage(file);
  ...
  async moveToCollection(
    ...
    this.logger.verbose(
      `User "${user.username}" Moving Collection ${moveToCollectionDto.sourceCollectionId} or Picto ${moveToCollectionDto.sourcePictoId} to ${moveToCollectionDto.targetCollectionId}`,
    );
    return this.collectionService.moveToCollection(
      user,
      moveToCollectionDto,
      fatherCollectionId,
    );
