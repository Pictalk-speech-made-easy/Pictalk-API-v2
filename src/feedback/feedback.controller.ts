...
export class FeedbackController {
  ...
  async createFeedback(
    ...
    this.logger.verbose(
      `Ccreating a feedback with title: ${createFeedbackDto.title}`,
    );
  ...
  async editFeedback(
    ...
    if (!user.admin) {
      throw new UnauthorizedException(
        `User ${user.username} is not admin, only admins can edit feedbacks`,
      );
    }
    this.logger.verbose(
      `Ccreating a feedback with title: ${editFeedbackDto.title}`,
    );
