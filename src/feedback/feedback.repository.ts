...
export class FeedbackRepository extends Repository<Feedback> {
  ...
  async getFeedback(
    ...
    if (searchFeedbackDto.sortField && searchFeedbackDto.sortOrder) {
      query.orderBy(
        `feedback.${searchFeedbackDto.sortField}`,
        searchFeedbackDto.sortOrder,
      );
    }
    try {
  ...
  async editFeedback(
    ...
    let { title, description, contact, action, blocking, evolution, state } =
      editFeedbackDto;
    feedback.action = action;
    feedback.evolution = evolution;
    feedback.blocking = blocking;
