...
export class PictoService {
    ...
    if (!picto) {
      ...
      if (
        ...
        throw new UnauthorizedException(
          `User ${user.username} does not have access to this picto`,
        );
      }
    }
  }
  ...
  async sharePictoById(
    ...
    const sharer = await this.authService.findWithUsername(
      sharePictoDto.username,
    );
    ...
    if (exists) {
      ...
      if (picto) {
        ...
        if (
          sharePictoDto.role === 'editor' &&
          !(picto.userId === user.id || editor != -1)
        ) {
          throw new UnauthorizedException(
            `${user.username} cannot share to ${sharer.username} as editor being a viewer youself`,
          );
        }
        const sharedWithMe = await this.collectionService.getCollectionById(
          sharer.shared,
          sharer,
        );
  ...
  async deletePicto(deletePictoDto: deletePictoDto, user: User): Promise<void> {
    ...
    if (deletePictoDto.fatherId) {
      ...
      fatherPictosIds.splice(
        fatherPictosIds.indexOf(deletePictoDto.pictoId),
        1,
      );
    ...
    try {
      ...
      if (result.affected === 0) {
        throw new NotFoundException(
          `Picto with ID "${deletePictoDto.pictoId}" not found`,
        );
      }
    } catch (error) {
      if (error.code === '23503') {
        ...
        throw new InternalServerErrorException(
          `couldn't delete picto with id ${deletePictoDto.pictoId}`,
        );
      }
    }
  }
  ...
  async modifyPicto(
    ...
    if (picto.userId === user.id || index != -1) {
      ...
      return this.pictoRepository.modifyPicto(
        picto,
        modifyPictoDto,
        user,
        filename,
      ...
      throw new UnauthorizedException(
        `User '${user.username}' is not authorized to modify this picto`,
      );
    }
  }
