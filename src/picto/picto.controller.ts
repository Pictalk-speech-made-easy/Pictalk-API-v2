...
export class PictoController {
  ...
  @ApiOperation({
    summary: 'share a picto with another user, with readonly or editor role',
  })
  async sharePictoById(
    ...
    for (let username of multipleSharePictoDto.usernames) {
      ...
      if (multipleSharePictoDto.access) {
        this.logger.verbose(
          `User "${user.username}" sharing Picto with id ${id} to User ${username} as ${multipleSharePictoDto.role}`,
        );
      } else {
        this.logger.verbose(
          `User "${user.username}" revoking access to Picto with id ${id} for User ${username}`,
        );
      }
      picto = await this.pictoService.sharePictoById(
        id,
        user,
        new sharePictoDto(
          multipleSharePictoDto.access,
          username,
          multipleSharePictoDto.role,
        ),
      );
    }
    return picto;
  ...
  async createPicto(
    ...
    if (!file) {
      this.logger.verbose(
        `User "${user.username}" tryed to create Picto without file or filename`,
      );
      ...
      if (IsValid(createPictoDto.meaning, createPictoDto.speech)) {
        if (createPictoDto.fatherCollectionId != user.shared) {
          ...
          const picto = await this.pictoService.createPicto(
            createPictoDto,
            user,
            filename,
          );
          const fatherCollection =
            await this.collectionService.getCollectionById(
              createPictoDto.fatherCollectionId,
              user,
            );
          ...
          if (createPictoDto.share != 0) {
            ...
            this.logger.verbose(
              `Auto sharing picto "${picto.id}" with viewers and editors`,
            );
          }
          return picto;
        } else {
  ...
  deletePicto(
    ...
    this.logger.verbose(
      `User "${user.username}" deleting Picto with id ${deletePictoDto.pictoId}`,
    );
  ...
  async modifyPicto(
    ...
    if (IsValid(modifyPictoDto.meaning, modifyPictoDto.speech)) {
      this.logger.verbose(
        `User "${user.username}" Modifying Picto with id ${id}`,
      );
      if (file) {
        const filename = await hashImage(file);
        return this.pictoService.modifyPicto(
          id,
          user,
          modifyPictoDto,
          filename,
  ...
  async copyPicto(
    ...
    await this.pictoService.copyPicto(
      copyPictoDto.fatherCollectionId,
      copyPictoDto.pictoId,
      user,
    );
